...
def system_timestamp():
    ...
    if i == -1:
        if j == 1:
            file.write(str('%.0f' % (time.time()*1000)) + ',' + 'Start' + ',' + '1' + '\n')
            button2['text']='Stop'
            label2['text']='Status:\nActive'
            window.update()
        if j == -1:
            file.write(str('%.0f' % (time.time()*1000)) + ',' + 'Stop' + ',' + '1' + '\n')
            response = messagebox.askyesno("Save Data", "Would you like to keep this data?")
            if response:
                file.write(str('%.0f' % (time.time()*1000)) + ',' + 'KeepOrDiscard' + ',' + '1' + '\n')
            else:
                file.write(str('%.0f' % (time.time()*1000)) + ',' + 'KeepOrDiscard' + ',' + '0' + '\n')
            button2['text']='Start'
            label2['text']='Status:\nIdle'
            window.update()
        j = j*-1
